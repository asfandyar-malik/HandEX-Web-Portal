class Insurance < ApplicationRecord
    ...
    has_attached_file :self_disclosure, styles: { medium: "300x300>", thumb: "100x100>" }
    validates_attachment_content_type :self_disclosure, content_type: ['image/jpeg', 'image/png', 'image/gif', 'application/pdf']

    has_attached_file :annual_financial_statement_importer, styles: { medium: "300x300>", thumb: "100x100>" }
    ...
    has_attached_file :credit_bureaus_importer, styles: { medium: "300x300>", thumb: "100x100>" }
    validates_attachment_content_type :credit_bureaus_importer, content_type: ['image/jpeg', 'image/png', 'image/gif', 'application/pdf']

    has_attached_file :environment_social_audit, styles: { medium: "300x300>", thumb: "100x100>" }
    validates_attachment_content_type :environment_social_audit, content_type: ['image/jpeg', 'image/png', 'image/gif', 'application/pdf']

    has_attached_file :project_memorandum_exporter_business, styles: { medium: "300x300>", thumb: "100x100>" }
    ...
    has_attached_file :additional_document, styles: { medium: "300x300>", thumb: "100x100>" }
